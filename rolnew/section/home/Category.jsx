      ...
      <div className="w-full flex flex-nowrap max-w-[800px] overflow-x-auto mx-auto sm:justify-center justify-start items-center scroll no-scroll">
        {
          categorys.map((category, i) => (
            <button type="button" className={`flex-none border-b-2 hover:bg-[#FFFFFF0D] delay-75 duration-500 ${i === activeCat ? 'border-primary bg-[#FFFFFF0D]' : 'border-[#B2B2B2] bg-transparent'} ${(i === 0 || i === categorys.length - 1) ? 'px-0' : 'px-3'} `} onClick={() => { setActiveTab(i); }} key={category.id}>
