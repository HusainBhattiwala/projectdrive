...
function GetStatusBtn(props) {
  ...
  switch (type) {
    ...
    case 'NO_SHOW':
      return (
        <Button className="!bg-[#888888] !bg-opacity-20 !text-[#757575] !text-[12px] !font-semibold !border-0 !h-[2rem] !py-0 !px-2 uppercase w-full">
          <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="7.5" cy="7.5" r="6.8" stroke="#888888" strokeWidth="1.4" />
            <path d="M12.0002 3.00019L3.13492 13.6775" stroke="#888888" strokeWidth="1.6" />
          </svg>
          NO SHOW
        </Button>
      );
...
function BookingCard({
  item, sendMail, sendMailLoader, setViewBookingId, setBookingActivity, setShowViewBooking, scrollToRef, newBookingId, setNewBookingId,
}) {
  ...
  const getBackGround = (type) => {
    switch (type) {
      ...
      case 'NO_SHOW':
        return '!border-[#888888] !shadow-[#888888] !bg-[#888888] hover:bg-[#888888] !bg-opacity-10  hover:bg-opacity-20';
      case 'CANCELLED':
  ...
  useEffect(() => {
    if (item.booking_ref_no === newBookingId) {
      const element = document.getElementById(`${item.booking_ref_no}`);
      element.scrollIntoView({ behavior: 'smooth', block: 'center' });
      setNewBookingId('');
    }
  }, [item, newBookingId, setNewBookingId]);

  return (
    <div className={`divide-x divide-gray-200 flex justify-items-center w-full py-5 mb-3 ${getBackGround(item?.ride_status)} mt-1 border-[0.60px] rounded-lg lg:flex-row flex-nowrap overflow-hidden`} key={item?.booking_id} ref={item.booking_ref_no === newBookingId ? scrollToRef : null} id={`${item.booking_ref_no}`}>
      <div className="basis-[20%] flex flex-col items-center justify-center relative box-border text-center px-2">
        <div className="text-neutral-700 ! font-bold py-2 underline text-xs">
          <button type="button" onClick={() => { setViewBookingId(item?.booking_id); setBookingActivity(1); setNewBookingId(item.booking_ref_no); setShowViewBooking(true); }}>
            #
            {item?.booking_ref_no}
          </button>
        </div>
        <div className="text-center py-2 leading-none">
          <div className={`${item?.booking_type === 'TRANSFERS' ? '!bg-primary !text-primary !bg-opacity-20' : '!bg-[#881B92] !text-[#881B92] !bg-opacity-20 !font-bold'} w-auto px-2 py-1 justify-center items-center gap-2.5`}>
            <P className="!text-xs font-semibold">{item?.booking_type}</P>
          </div>
          <div className="w-auto py-2 my-2 !text-xs font-medium px-2 bg-[#DFE2F4] text-[#232C68] rounded-[2px]">
            <span>By </span>
            <span>{item?.booker_name}</span>
            <br />
            <span>{formatDateTime(item?.travel_date)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="basis-[20%] flex justify-center items-center flex-col px-2">
        ...
        {/* <div className="w-24 h-6 px-8 py-1 bg-red-700 bg-opacity-20 justify-center items-center gap-2.5 inline-flex"> */}
        {/*  <P className="text-center uppercase text-red-700 text-xs font-semibold">DUE</P> */}
        {/* </div> */}
        <div className={`h-6 px-8 py-1 my-2 ${item?.payment_type === 'PAID' ? 'bg-emerald-100' : 'bg-[#FFDEDE]'} justify-center items-center gap-2.5 inline-flex`}>
          <P className={`text-center uppercase ${item?.payment_type === 'PAID' ? 'text-green-600' : 'text-[#DA1A1A]'} text-xs font-semibold`}>{item?.payment_type === 'PAYMENT_LINK' ? 'DUE' : item?.payment_type}</P>
        </div>
      </div>
      <div className="basis-[25%] flex justify-center items-center flex-col px-2">
        ...
        <div className="flex gap-[25px] flex-row my-2">
          <button type="button" onClick={() => { setViewBookingId(item?.booking_id); setBookingActivity(0); setNewBookingId(item.booking_ref_no); setShowViewBooking(true); }} className="w-[16.599px] h-[16.077px] relative">
