...
export default function BookerDetails({
  ...
  function getPhoneNumber() {
    ...
    if (!userExists) {
      if (showBooker && userDetails?.usermobileno) {
        ...
        setValue(
          'bookercountrycode',
          userDetails?.usercountrycode.replace('+', ''),
        );
        return mobile;
      }
    } else if (showBooker) {
      ...
      setValue(
        'bookercountrycode',
        user?.passenger_country_code.replace('+', ''),
      );
      return mobile;
    }
    return '+44';
  ...
  return (
    <>
      <div
        className={`${!showBooker ? 'col-span-2' : 'col-span-2 sm:col-span-1'}`}
      >
      ...
      <div className="relative col-span-2 md:md:col-span-1">
        <input
          {...register('bookerfname', {
            required: 'Booker First Name is required',
            autoComplete: 'off',
          })}
          type="text"
      ...
      <div className="relative col-span-2 md:md:col-span-1">
        <input
          {...register('bookerlname', {
            required: 'Booker Last Name is required',
            autoComplete: 'off',
          })}
          type="text"
      ...
      <div className="relative col-span-2 md:md:col-span-1">
        <input
          {...register('bookeremail', {
            required: 'Booker Email is required',
            autoComplete: 'off',
          })}
          type="text"
          placeholder="Booker Email"
          className={`w-full input focus:outline-none bg-[#223544D9]  text-[#B2B2B2] border !border-[#828282] ${
            (!!userDetails?.useremailid || !!session?.user.email)
            && 'pointer-events-none'
          }`}
          readOnly={!!userDetails?.useremailid || !!session?.user.email}
          defaultValue={
            showBooker ? session?.user.email || userDetails?.useremailid : ''
          }
          autoComplete="new-password"
          onBlur={(e) => {
