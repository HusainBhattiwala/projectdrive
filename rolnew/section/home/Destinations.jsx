...
function Destinations(destinationData = null) {
  return (
    <Container className='bg-[#223544] py-20 text-center'>
      <Title
        subTitle={destinationData?.subTitle ? destinationData.subTitle : 'Popular trips'}
        mainTitle={destinationData?.title ? destinationData.title : 'Explore Our Most Sought-After Destinations'}
      />
                  ...
                  <div className='px-6 py-4 bg-white bg-opacity-10 flex items-center justify-start gap-x-3 rounded-xl'>
                    <div className='bg-white bg-opacity-10 p-2.5 rounded-lg'>
                      <div className='w-5 h-4'>
                        <Pic src='/rolnew/global/icons/heart.svg' alt='heart' objectFit='cover' />
                      </div>
                    </div>
                    <div className='flex gap-x-0.5 items-center'>
                      <p className='text-[#B2B2B2] font-normal text-base'>{destination.from}</p>
                      {destination?.isFromTo &&
                        <>
                          <div className='w-4 h-4'>
                            <Pic src='/rolnew/global/icons/small_arrow.svg' alt='small_arrow' objectFit='cover' />
                          </div>
                          <p className='text-[#B2B2B2] font-normal text-base'>{destination.to}</p>
                        </>
                      }
                    </div>
                  </div>
              ...
              cityPairs.map((cityPair) => (
                <div key={cityPair.from} className='col-span-1'>
                  <div className='px-6 py-4 bg-white bg-opacity-10 flex items-center justify-start gap-x-3 rounded-xl'>
                    ...
                    <div className='flex gap-x-0.5 items-center'>
                      <p className='text-[#B2B2B2] font-normal text-base'>{cityPair.from}</p>
                      <div className='w-4 h-4'>
                        <Pic src='/rolnew/global/icons/small_arrow.svg' alt='small_arrow' objectFit='cover' />
                      </div>
                      <p className='text-[#B2B2B2] font-normal text-base'>{cityPair.to}</p>
                    </div>
                  </div>
                </div>
              ))
